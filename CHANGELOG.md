...
### Performance
- **Restored `modulepreload` for `framer-motion` and `react-router`** (`scripts/prerender.ts`): Removed the `vendorChunksToDefer` block that was stripping `<link rel="modulepreload">` for these chunks. The intent was to reduce the "Unused JavaScript" diagnostic, but the effect was creating a 3-step sequential download chain (`index.js → framer-motion → react-router`) adding 316ms of critical-path latency. Both chunks are now preloaded in parallel with all other vendor assets. The `react-dom` entry in that list was also a no-op (the chunk is named `react-*`, not `react-dom-*`) so the entire block was dead weight.
- **Eliminated LCP element render delay** (`src/components/Hero.tsx`): Removed `initial={{ opacity: 0, ... }}` from all motion elements in the Hero component. Framer Motion serializes the `initial` state as inline styles during SSG prerendering, causing the hero heading, badge, paragraph, and CTA to be baked into the static HTML as `style="opacity:0"`. Lighthouse measured a 1,420ms "element render delay" on the LCP element (`<h1>Build Amazing Web Apps Fast</h1>`) because it was invisible until JS loaded and the animation completed. Removing `initial` causes Framer Motion to render elements in their visible state during SSR, so the pre-rendered HTML content is immediately paintable. Hover and scroll-triggered animations (`whileHover`, `whileInView`) are unaffected.
- **Eliminated LoadingScreen flash on homepage** (`src/Router.tsx`): Changed `Home` from a `React.lazy()` import to a direct static import. `React.lazy()` always suspends for at least one render tick even when the chunk is already preloaded, causing the Suspense fallback (`LoadingScreen`) to flash between the SSG HTML and the React-rendered content. With a direct import, React renders `Home` synchronously on mount — no flicker. `About` and `SuspenseExample` remain lazy since they are only loaded on navigation, not on initial page load.
